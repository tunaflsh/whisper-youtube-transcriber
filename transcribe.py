...
def split(transcript, split_size):
    ...
    for i, j in zip(chunk_ids[:-1], chunk_ids[1:]):
        chunks.append({
            ...
            'tokens': sum([len(encoding.encode(segment['text']))
                           for segment in transcript['segments'][i:j]]),
            'text': ''.join([segment['text'] for segment in transcript['segments'][i:j]]),
        })
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Transcribe an audio file to text using the OpenAI API.')
    parser.add_argument('file', type=str, help='Path to the audio file to transcribe, in one of these formats: mp3, mp4, mpeg, mpga, m4a, wav, or webm.')
    parser.add_argument('-p', '--prompt', type=str, help="An optional text to guide the model's style or continue a previous audio segment. The prompt should match the audio language.", default=None)
    parser.add_argument('-l', '--language', type=str, help="The language of the input audio. Supplying the input language in ISO-639-1 format will improve accuracy and latency.", default=None)
    parser.add_argument('-t', '--translate', action='store_true', help="Translate the audio file to English.")
    parser.add_argument('-s', '--split', type=int,
                        help="Create a json file splitting the transcript into chunks of {split} tokens or less. The format of the output file is: {'split': int, 'num_chunks': int, 'total_tokens': int, 'chunks': [{'chunk_id': int, 'start': float, 'end': float, 'tokens': int, 'text': str}, ...]}", default=None)
    ...
    if extension in ['.mp3', '.mp4', '.mpeg', '.mpga', '.m4a', '.wav', '.webm']:
        if args.translate:
            ...
            transcript = translate_audio_to_english(
                args.file,
                args.prompt
            )
            base_name += '[English]'
